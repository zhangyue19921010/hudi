...
/**
 * JDCompactionCommitSink is the same as CompactionCommitSink except how to deal with "clean.after.compact.enabled"
 * in JDCompactionCommitSink, if "cclean.after.compact.enabled" was false, it means we would disable clean totally no matter it's async or not.
 *
 */
public class JDCompactionCommitSink extends CompactionCommitSink {
  private static final Logger LOG = LoggerFactory.getLogger(JDCompactionCommitSink.class);
  ...
  private final boolean cleanAfterCompact;
  ...
  public JDCompactionCommitSink(Configuration conf) {
    super(conf);
    this.conf = conf;
    this.cleanAfterCompact = conf.getBoolean(FlinkOptions.CLEAN_AFTER_COMPACT_ENABLED);
  }

  @Override
  public void open(Configuration parameters) throws Exception {
    if (cleanAfterCompact) {
  ...
  private void doCommit(String instant, Collection<CompactionCommitEvent> events) throws IOException {
    ...
    if (cleanAfterCompact && !conf.getBoolean(FlinkOptions.CLEAN_ASYNC_ENABLED) && !isCleaning) {
