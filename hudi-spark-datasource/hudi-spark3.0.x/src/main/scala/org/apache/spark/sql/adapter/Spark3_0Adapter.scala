...
import org.apache.spark.sql.parser.HoodieSpark3_0ExtendedSqlParser
...
class Spark3_0Adapter extends BaseSpark3Adapter {
  ...
  override def createExtendedSparkParser: Option[(SparkSession, ParserInterface) => ParserInterface] = {
    ...
    Some(
      (spark: SparkSession, delegate: ParserInterface) => new HoodieSpark3_0ExtendedSqlParser(spark, delegate)
    )
  }
